...
class ReplyMock(MockLLMProvider):
    """Proveedor mock que devuelve un JSON de respuesta sugerida válido."""
    ...
    def complete(self, **kwargs):
        ...
        if self._bad_json:
            ...
            content = json.dumps(
                {
                    "suggestedReply": "Gracias por escribirnos. Hemos revisado su caso y lo estamos escalando al equipo técnico para su resolución.",
                    "confidence": 0.9,
                    "sources": ["descripción", "historial"],
                    "policyFlags": [],
                    "warnings": [],
                }
            )
...
@pytest.fixture
def reply_provider(monkeypatch):
...
def _suggest(client: TestClient, tokens: dict, ticket_id: int, body: dict | None = None):
    return client.post(
        f"/v1/ai/tickets/{ticket_id}/suggested-reply",
        json=body,
        headers=_headers(tokens),
    )


def test_reply_success(client: TestClient, reply_provider) -> None:
    ...
    assert body["sources"] == ["descripción", "historial"]
...
def test_reply_with_tone_and_language(client: TestClient, reply_provider) -> None:
    ...
    ticket = _create_ticket(client, tokens)
    resp = _suggest(client, tokens, ticket["id"], {"tone": "empático", "language": "en"})
    assert resp.status_code == 200, resp.text
    assert resp.json()["suggested_reply"]


def test_reply_requires_token(client: TestClient) -> None:
    assert client.post("/v1/ai/tickets/1/suggested-reply").status_code == 401


def test_reply_other_tenant_is_404(client: TestClient, reply_provider) -> None:
    reply_provider(ReplyMock())
    ...
    resp = _suggest(client, tokens_b, ticket["id"])
    assert resp.status_code == 404


def test_reply_low_confidence_adds_warning(client: TestClient, reply_provider) -> None:
    reply_provider(ReplyMock(payload={
        "suggestedReply": "Necesitamos más información para ayudarle.",
        "confidence": 0.2,
        "sources": [],
        "policyFlags": [],
        "warnings": [],
...
def test_reply_llm_down_returns_503(client: TestClient, reply_provider) -> None:
    reply_provider(ReplyMock(fail=True))
...
def test_reply_invalid_json_returns_422(client: TestClient, reply_provider) -> None:
    reply_provider(ReplyMock(bad_json=True))
...
def test_reply_persists_suggestion_without_pii(client: TestClient, reply_provider) -> None:
    reply_provider(ReplyMock())
...
def test_reply_audits_and_metrics(client: TestClient, reply_provider) -> None:
    ...
    with SessionLocal() as db:
        events = db.query(AuditEvent).filter(AuditEvent.action == "ai.replied").all()
        assert len(events) == 1
        event = events[0]
    ...
    assert resp.status_code == 200
    assert "ai_replies_total" in resp.text
