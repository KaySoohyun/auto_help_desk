...
- [ ] **T1.3** Crear modelo `UserTenant` en `app/models/user_tenant.py` (user_id FK, tenant_id FK, role, created_at) — **DEFERIDO: se hará después de las tareas actuales**
...
- [ ] **T1.7** Agregar relación `tenants` en modelo `User` (many-to-many via `user_tenants`) — **DEFERIDO**
...
- [ ] **T1.12** Script de migración: crear tabla `user_tenants` y migrar datos de `users.tenant_id` — **DEFERIDO**
- [ ] **T1.13** Script de migración: eliminar columna `users.tenant_id` — **DEFERIDO**
...
- [x] **T1.19** Tests de modelos y migraciones (260 tests pasan)
